...
/*
 This buggy file should be replaced with a cordic solution. (see cartesian2polar_new.v).
 
 One issue in the original file is that for small i_x, i_y the number of iterations (32) is too small to
 allow the correct theta and rho values to be found.
 
 To address this it is necessary to catch when we have small values and then pre-scale.
...
reg [15:0] amp;
   ...
   //ECO: Add scaling factor (number of bits to left-shift for small ips)

   //register the scale shift
   reg [3:0] scale_shift; //upto 15 bits (since ips are 16 bit)
    
   wire [24:0] mul = {15'd0,a} * {8'd0,absx}; //ECO widen to 25 bits

   //
   //ECO: Wire for max abs to compute leading zeros
   //    

   wire [15:0] max_abs = (absx > absy) ? absx: absy;


   //ECO: Widen to 17 bits   
   wire [16:0] amp_w = {2'b0,mul[23:10]} + {1'b0,absx};
   wire [16:0] ampatt_w = amp_w >> ATTENUAION;

   //ECO: Compute scale shift based on max_abs to
   //approximately normalize to 16384 range

   reg [3:0]   calc_shift; //local var to calculate scaling shift
   always @* begin
    calc_shift = 4'd0;
    if (max_abs != 0) begin
        while ((max_abs << calc_shift) < 16'd8192 && calc_shift < 4'd13) 
            calc_shift = calc_shift + 4'd1;
    end
   end
   

   wire [15:0] scaled_absx = absx << scale_shift;
   wire [15:0] scaled_absy = absy << scale_shift;
    ...
    if(~rstn) begin
       ...
       scale_shift <= 0;
        ...
        if(cnt==5'd0) begin
            ...
            if(i_en)
                cnt <= 5'd31; //ECO: added one more stage to allow
	   //latching shift
        end else begin
            cnt <= cnt - 5'd1;
	   if (cnt == 5'd31)
	     begin
		scale_shift <= calc_shift;
	     end
	   
           else if(cnt>=5'd30) begin
	       //ECO
                signxy <= scaled_absx < scaled_absy;
                if(scaled_absx < scaled_absy) begin
                    absx <= scaled_absy; //ECO scale
		    absy <= scaled_absx; //Eco scale
                    smtb <= {scaled_absy, 12'h0}; //ECO
                    smta <= {scaled_absx, 12'h0}; //ECO
                end else begin
		   absx <= scaled_absx; //ECO scale
		   absy <= scaled_absy; //ECO scale
                    smtb <= {scaled_absx, 12'h0}; //ECO scale
                    smta <= {scaled_absy, 12'h0}; //ECO scale
		end // else: !if(scaled_absx < scaled_absy)
                ...
                amp <= ampatt_w >> scale_shift ; //ECO: De-Scale rho back down
                if(signx)
                    theta <= 12'd2048 - theta;
            end else if(cnt==5'd1) begin
                o_en <= 1'b1;
	       //ECO: Allow 0 min 1 for tiny values
               o_rho <= (amp > 16'd4095) ? 12'd4095 : 
			(amp[11:0]==0 ? 12'd0:
			 (amp[11:0] < 12'd1: 12'd1:amp[11:0]));
	       
	       
                o_theta <= signy ? 12'd0-theta : theta; //This underflows to positive
            end
        end
    end // else: !if(~rstn)
