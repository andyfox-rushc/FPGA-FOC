...
// Scaling variables (regs for latching)
reg [4:0] shift_amt;
reg [15:0] max_abs;

// Combinational temps for fresh input processing
wire [15:0] absx_temp = (i_x < $signed(16'd0)) ? -i_x : i_x;
wire [15:0] absy_temp = (i_y < $signed(16'd0)) ? -i_y : i_y;
wire [15:0] max_abs_temp = (absx_temp > absy_temp) ? absx_temp : absy_temp;

// Combinational block for computing shift_amt (leading zero count, capped at 14)
reg [4:0] computed_shift;
always @(*) begin
    integer i;
    reg [15:0] val = max_abs_temp;  // Use combo temp for fresh value
    computed_shift = 5'd0;
    if (val != 0) begin
        for (i = 15; i >= 0; i = i - 1) begin
            if (val[i] == 1'b0) begin
                computed_shift = computed_shift + 1;
            end else begin
                break;  // Stop once we find the first 1 (from MSB)
            end
        end
        if (computed_shift > 5'd14) computed_shift = 5'd14;
    end
end
...
always @ (posedge clk or negedge rstn)
    if (~rstn) begin
        ...
        if (cnt == 5'd0) begin
            ...
            if (i_en) begin
                cnt <= 5'd30;
                // Latch from combo temps
                absx <= absx_temp;
                absy <= absy_temp;
                max_abs <= max_abs_temp;
                shift_amt <= computed_shift;
                // Apply scaling (using latched values, but since <=, it will apply on next edge; that's fine as long as used later)
                absx <= absx_temp << computed_shift;
                absy <= absy_temp << computed_shift;
            end
        end else begin
            cnt <= cnt - 5'd1;
            if (cnt >= 5'd30) begin
                ...
                amp <= (amp_w >> shift_amt) >> ATTENUAION;
                if (signx)
...
always @ (posedge clk)
  begin
  //defaults
  rom_a <= 9'd0;     // Placeholder
  rom_theta <= 9'd0; // Placeholder
